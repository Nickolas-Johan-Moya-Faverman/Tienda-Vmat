...
class Clientes extends Controller
{
    ...
    public function registroDirecto()
    {
        if (isset($_POST['nombre']) && isset($_POST['clave'])) {
            if (empty($_POST['nombre'] || $_POST['correo'] || $_POST['clave'] || $_POST['telefono'])) {
                $mensaje = array('msg' => '|| TODOS LOS CAMPOS NECESITAN SER LLENADOS ||', 'icono' => 'warning');
            } else {
                $nombre = $_POST['nombre'];
                $correo = $_POST['correo'];
                $clave = $_POST['clave'];
                ...
                if (empty($verificar)) {
                    ...
                    $data = $this->model->registroDirecto($nombre, $telefono, $correo, $hash, $token);
                    if ($data > 0) {
                        $_SESSION['correoCliente'] = $correo;
                        $_SESSION['nombreCliente'] = $nombre;
                        $mensaje = array('msg' => 'USTED SE HA REGISTRADO CON EXITO', 'icono' => 'success', 'token' => $token);
                    } else {
                        $mensaje = array('msg' => 'ERROR AL REGISTRARSE', 'icono' => 'error');
                    }
                } else {
                    $mensaje = array('msg' => 'USTED YA TIENE UNA CUENTA EXISTENTE', 'icono' => 'warning');
                }
            }
    ...
    public function enviarCorreo()
    {
        if (isset($_POST['correo']) && isset($_POST['token'])) {
            ...
            try {
                ...
                //Recipients
                $mail->setFrom('freddyferreirasaca@gmail.com', TITLE);
                $mail->addAddress($_POST['correo']);

                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'Mensaje desde la empresa ' . TITLE;
                $mail->Body    = 'Para verificar tu correo electronico en nuestra pagina <a href="' . BASE_URL . 'clientes/verificarCorreo/' . $_POST['token'] . '">HAZ CLICK AQUI</a>';
                $mail->AltBody = 'Gracias por unirte a nuestra familia VMAT';
            ...
            if (empty($_POST['correoLogin'] || $_POST['claveLogin'])) {
                ...
                if (!empty($verificar)) {
                    if ($verificar['estado'] == 0) {
                        $mensaje = array('msg' => 'USTED ESTA INHABILITADO', 'icono' => 'warning');
                    } else {
                        if (password_verify($clave, $verificar['clave'])) {
                            $_SESSION['correoCliente'] = $verificar['correo'];
                            $_SESSION['nombreCliene'] = $verificar['nombre'];
                            $mensaje = array('msg' => 'SE LOGEO CORRECTAMENTE', 'icono' => 'success');
                        } else {
                            $mensaje = array('msg' => 'CONTRASEÑA INCORRECTA, REVISE LOS DATOS INGRESADOS', 'icono' => 'error');
                        }
                    }
                } else {
    ...
    public function registrarPedido()
    {
        ...
        if (is_array($pedidos) && is_array($productos)) {
            ...
            $nombre = $pedidos['payer']['name']['give_name'];
...
//sb-skl2c15279554@personal.example.com
//XzV@$z2b