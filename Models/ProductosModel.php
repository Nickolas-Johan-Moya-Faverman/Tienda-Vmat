...
class ProductosModel extends Query{
    ...
    {
        $sql = "SELECT * FROM productos1 WHERE estado = $estado";
    ...
    {
        $sql = "INSERT INTO productos1 (nombre, descripcion, precio, cantidad, imagen, id_categoria) VALUES (?,?,?,?,?,?)";
        $array = array($nombre, $descripcion, $precio, $cantidad, $imagen, $categoria);
        return $this->insertar($sql, $array);
    }
    public function eliminar($idPro)
    {
        $sql = "UPDATE productos1 SET estado = ? WHERE id = ?";
        $array = array(0, $idPro);
        return $this->save($sql, $array);
    }
    public function getProducto($idPro)
    {
        $sql = "SELECT * FROM productos1 WHERE id = $idPro";
        return $this->select($sql);
    }
    public function modificar($nombre, $descripcion, $precio, $cantidad, $destino, $categoria, $id)
    {
        $sql = "UPDATE productos1 SET nombre = ?, descripcion = ?, precio = ?, cantidad = ?, imagen = ?, id_categoria = ? WHERE id = ?";
