...
if (!empty($_POST)) {
  ...
  if (empty($_POST['nombre']) || empty($_POST['telefono']) || empty($_POST['direccion'])) {
    ...
    if (is_numeric($dni) and $dni != 0) {
      ...
      $query = mysqli_query($conexion, "SELECT * FROM cliente where (ci = '$dni' AND idcliente != $idcliente)");
    ...
    if ($resul >= 1) {
      ...
      $sql_update = mysqli_query($conexion, "UPDATE cliente SET ci = $dni, nombre = '$nombre' , telefono = '$telefono', direccion = '$direccion' , correo = '$correo' WHERE idcliente = $idcliente");
...
?>
      ...
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Editar datos de cliente</h1>
