...
if (!empty($_POST)) {
    ...
    if (empty($_POST['nombre']) || empty($_POST['telefono']) || empty($_POST['direccion']) || empty($_POST['correo'])) {
        ...
        if (is_numeric($dni) and $dni != 0) {
            $query = mysqli_query($conexion, "SELECT * FROM cliente where ci = '$dni'");
        ...
        if ($result > 0) {
            ...
            $query_insert = mysqli_query($conexion, "INSERT INTO cliente(ci,nombre,telefono,direccion,correo) values ('$dni', '$nombre', '$telefono', '$direccion', '$correo')");
            if ($query_insert) {
